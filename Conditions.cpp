...
void lab6Conditions() {
	...
	cout << ">>> 5) «Мячик на лесенке». На вершине лесенки, содержащей n (0<n<31) ступенек, \nнаходится мячик, который начинает прыгать по ним вниз, к основанию. Мячик \nможет прыгнуть на следующую ступеньку, на ступеньку через одну или через 2. (То \nесть если мячик лежит на 8-й ступеньке, то он может переместиться на 5, 6 или 7-ю.) \nОпределить число всевозможных «маршрутов» мячика с вершины лесенки \nна землю.\n\n";
	cout << ">>> 6) Дана матрица A[1..n,1..m] (0<n≤1000, 0<m≤1000). Элементами матрицы являются целые числа, по модулю не превосходящие 103. Подматрица – это часть матрицы из l строк и  k столбцов (0<l≤n, 0<k≤m). Требуется найти количество подматриц матрицы A, сумма элементов которых в точности равна заданному числу s.\n\n";
...
void lab7Conditions() {
	...
	cout << "Лаба №7\n\n";
	cout << ">>> 1) Разработать программу поиска решения задачи о «магическом квадрате» третьего \nпорядка методом полного перебора.\n\n";
	cout << ">>> 2) Преобразуй и властвуй. Даны n целых чисел и ряд из n коробок. Между коробками \nфиксировано расположены знаки неравенств. Разработайте программу, которая \n«помещает» числа в коробки так, чтобы неравенства были правильные.\n\n";
	cout << ">>> 3) Задача интервального разбиения. В задаче интервального планирования \nиспользуется один ресурс и много заявок в форме временных интервалов; требуется \nвыбрать, какие заявки следует принять, а какие отклонить. В задаче интервального \nразбиения используется несколько идентичных ресурсов, для которых необходимо \nраспланировать все заявки с использованием минимально возможного количества \nресурсов. Найдите жадный алгоритм решения и реализуйте его.\n\n";
	cout << ">>> 4) Разработать программную реализацию алгоритма поиска k-ой статистики за \nлинейное в среднем время.\n\n";
	cout << ">>> 5) Разработать программную реализацию алгоритма поиска k-ой статистики за \nлинейное в худшем случае время. \n\n";
	cout << ">>> 6) Поиск k-ой порядковой статистики в двух массивах. Даны два отсортированных \nмассива a и b размерами n и m соответственно. Требуется найти k-ый порядковый \nэлемент после их слияния. Считается, что все элементы в массивах различны и \nнумеруются с нуля. \n\n";
