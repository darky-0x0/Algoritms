...
void lab4task7() {
	...
	int N = 8; //размерность пол€
	
	int count(2); //счЄтчик рекурсии

	pair<int, int> knight; //позици€ кон€

	pair<int, int> startPosition(knight); //стартова€ позици€ кон€ 

	pair<int, int> king; //позици€ корол€
	...
	if (isEndAttila == true) {
		...
		cout << "\nѕути, который соответствует условию задачи, нет\n";
...
void AttilaKnight(vector<vector<int>>& field, int size, pair<int, int>& knight, pair<int, int> const startPosition, pair<int, int> const king, vector<vector<int>>& result, int count) {
	...
	if ((kingDefeated == true) && ((knight.first == startPosition.first) && (knight.second == startPosition.second))) { //если путь проходит через клетку корол€ и мы вернулись к началу
		...
		for (int i = 0; (i < 8) && (isEndAttila == false); i++) { //перебор возможных шагов
			...
			//заходим, если клетка пуста€ || заходим на начальную позицию, если король повержен
			if ((field[newIndex.first][newIndex.second] == 0) || ((field[newIndex.first][newIndex.second] == 1) && kingDefeated == true)) { 
				...
				if ((newIndex.first == king.first) && (newIndex.second == king.second)) kingDefeated = true; //помечаем, если повергли корол€
				...
				if ((newIndex.first == king.first) && (newIndex.second == king.second)) kingDefeated = false; //откат на случай перепросчЄта ходов
